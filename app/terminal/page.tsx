      ...
      <div className="pt-14 px-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-950/40 p-6">
          <h3 className="text-xl mb-4">
            {step === 'pin' ? '📈 Verbrauch aller Spieler' : '📈 Dein Wochenverbrauch'}
          </h3>
          <div className="space-y-2">
            {(step === 'pin' ? allWeek : myWeek).map((w) => {
              const max = step === 'pin' ? maxAll : maxMy
              const pct = Math.max((w.qty / (max || 1)) * 100, 5)
              return (
                <div key={w.name}>
                  <div className="text-xs text-neutral-400 mb-1">{w.name}</div>
